...
| **出生模型切换** | 上下文栏两个 `disabled` 占位按钮（「切换出生模型」「切换预测周期」） | **真实重新分析**：`listing_open` / `ipo_date` 可切（后者带 C 级数据质量说明），`company_foundation` / `first_trade` 缺数据源 → 显示为**不可用 + 原因**（后端按契约 422，不猜测）；演示样本冻结在默认基准上，换基准**显式拒绝**而不是拿冻结样本冒充 |
| **研究窗口（登记）** | 同上（占位） | 可切（20d/60d），值登记进 `analysis_run.horizon` 并回传；**界面写明"不改变三个模型的分数"** —— 该字段不参与因子计算，包装成"预测周期切换"是虚假能力宣称 |
| **请求超时** | 无（`fetch` 没有内置超时） | 30 秒 AbortController 超时；后端挂起时给出可重试的错误，而不是永远停在"加载中" |
...
| 前端全量 E2E | `SMP_WEB_BASE=http://127.0.0.1:3111 SMP_E2E_LIVE_API=1 SMP_E2E_REAL_BASE=http://127.0.0.1:3111 npx playwright test` | **161 passed / 0 failed / 0 skipped**（上轮 119 passed + 4 skipped；本轮 live 组真实执行，并新增真实模式 8 项、故障边界 5 项、出生模型切换 7 项、双视口 13 项） |
...
### 1.1 十页交互清单（入口 → 操作 → 状态变化/请求 → 用户可见结果 → 失败反馈）

| 页面 | 交互 | 入口 | 状态变化 / 请求 | 用户可见结果 | 失败反馈 |
|---|---|---|---|---|---|
| 01 首页 | 搜索标的 | 顶部搜索框 | 输入即建议（`GET /stocks/search`）；提交后跳综合页 | 建议列表含代码/名称/交易所；跳转后上下文栏显示该标的 | 无匹配给空列表；网络失败显示错误 |
| 01 首页 | 进入功能页 | 左侧导航（8 项） | 客户端路由 | 目标页首屏 | 页面级错误态 |
| 02 综合研判 | 切换出生模型 | 上下文栏「详情」→ 出生模型 | 改 URL → `POST /analysis/multi`（带 `birth_basis`） | 出生时刻、四柱、紫微盘面、三模型分数全部更新；C 级质量有说明 | 不可用基准禁用 + title 说明原因；演示模式换基准 → 隔离卡 |
| 02 综合研判 | 研究窗口 | 同上下「研究窗口」 | 改 URL → 重新分析（登记 horizon） | 窗口标签更新；**分数不变**（界面已写明） | 同上（无失败路径，纯登记） |
| 02 综合研判 | 导出报告 | 页眉「导出报告」/ 上下文栏同一按钮 | `GET /analysis/{id}/report?format=...`（真实）/ 前端生成（演示） | 三种格式实际下载：Markdown / 可打印 HTML / 结构化 JSON | 失败显示 `export-feedback` 文本（含超时文案） |
| 02 综合研判 | 证据抽屉 | 关键证据卡 | 本地展开 | 支持/反证两侧内容 | Esc 关闭；空态说明 |
| 02 综合研判 | 切换股票 | 上下文栏「切换股票」 | 弹窗搜索 → 路由跳转 | 全页数据换成新标的 | 无结果空列表 |
| 03 八字详情 | 查看四柱/五行/时间结构 | 页面区块 | 无额外请求（首屏已加载） | 四柱表、五行条、流年流月流日 | 数据缺失显示"—" |
| 04 紫微斗数 | 顺逆切换 | 盘面卡头 `VariantSwitcher` | 重新分析（`variant_mode` 变） | 十二宫/四化/流年流月随真实排盘变化 | 紫微不可用 → `UnavailableBlock` + 重排盘按钮 |
| 04 紫微斗数 | 宫位明细 | 每宫「明细」按钮 | 本地弹层（无请求） | 该宫全部主星/辅星/杂曜 + 庙旺 + 长生十二神 + 三方四正 | 空宫如实写「不用其它宫补齐」 |
| 05 历史验证 | 选择实验 | 实验下拉 | 本地切换（数据已在首屏加载） | 持有期对比图 + 实验明细随之切换 | `INVALID_CONTROL` 显式警告条 |
| 06 因子字典 | 搜索 / 筛选 / 分页 | 列表上方 | `GET /factor-dictionary`（按 engine/category）+ 本地文本筛选 | 列表与详情更新；页面写明筛选范围 | 无结果显示空态 |
| 07 模型分歧 | 场景切换 | 场景切换器（演示模式） | 本地切换三份真实组装结果的冻结样本 | 冲突摘要/矩阵/归因随之变化 | 引擎不可用 → 不计入共识分母并标注 |
| 08 黄历 | 粒度切换 / 点击日期卡 | 网格上方档位 / 卡片 | `GET /huangli/outlook`（mode/days/months） | 日期卡网格与当天干支/宜忌联动 | 日历越界 → 部分覆盖说明，不补造日期 |
| 08 黄历 | 历史表现窗口/持有期 | 面板内选择器 | `GET /huangli/performance`（window/horizon） | 统计随参数变化，口径披露常驻 | 样本不足 → 空态 + 原因 |
| 09 古籍证据 | 检索 / stance 页签 | 检索框 / 页签 | 本地结果集内筛选（范围已在页面写明） | 结果列表按 stance 过滤 | 无命中空态 |
| 09 古籍证据 | 选中证据 | 列表项 | 本地 | 原文 → 项目解释 → 关联主题与因子 | — |
| 10 时间窗口 | 粒度切换 | 卡片头「逐日/月度」 | `GET /timeline/days\|weeks\|months` | 主视图与明细切换（逐日 20/月 12/周 12） | 分区失败 → `timeline-*-error` + 「重试本区」 |
| 10 时间窗口 | 图表联动 | 点击热力图日期 / 周排名行 | 本地 | 右侧解释显示该日/该周的原始结果与风险 | 未选中 → 「窗口解释：暂无数据」 |
| 10 时间窗口 | 审计入口 | 页面底部 | 本地展开 | 原始表格（标题数量与实际行数一致） | — |
| 全部股票页 | 重新计算 | 上下文栏 | `POST /analysis/multi`（绕过缓存） | Loading → 新结果 | 失败 → 页级错误 + 重试；请求超时 30 秒 |
| 全部股票页 | 本机自选 | 上下文栏星标 / 「本机自选 N」 | localStorage 写入（无网络） | 加入/移除/去重/刷新恢复；面板写明「本机、不跨设备同步」 | 隐私模式 → 内存兜底，功能不失效 |

**说明**：本表是"入口 → 操作 → 请求 → 可见结果 → 失败反馈"的逐项对照，
其中每一项都有对应的 E2E 或既有 spec 覆盖（见 §1 与 `apps/web/e2e/`）。
...
### 2.3 逐页目视记录（本轮已实际打开全部十页对照图）

**十页对照图均已实际打开逐页检查**，逐页的一致点、差异点与差异归属见
[`docs/UI_FINAL_VISUAL_REVIEW.md`](UI_FINAL_VISUAL_REVIEW.md)（独立文件，避免把目视判断
与几何门禁混在一张表里）。差异最大的三页是 05 历史验证 / 08 黄历 / 10 时间窗口，
共同原因是参考图渲染了本平台**没有契约**或**研究纪律不允许**的内容
（策略净值、逐日胜率与仓位建议、插值预测曲线、多维度雷达）。

差异分三类，均已在目视记录中逐条标注：
1. **数据契约差异** —— 接口未提供该字段 → 空态 + 说明原因；
2. **研究纪律差异** —— 有数据但不得那样呈现（不平均掩盖冲突、不插值成预测、不补第三级吉凶、反证排前）；
3. **设计取舍差异** —— 数据相同、呈现方式不同（紫微网格环、证据检索范围、离散观点带）。
